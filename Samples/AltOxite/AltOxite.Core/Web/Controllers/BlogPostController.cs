...
namespace AltOxite.Core.Web.Controllers
{
    public class BlogPostController
    {
        public BlogPostViewModel Index(BlogPostSetupViewModel inModel)
        {
            ...
            var commentCount = inModel.Post.Comments.Count();

            var tagLinksAndCommentsLink = ((tagLinks.Length != 0) ? "Filed under {0}| ".ToFormat(tagLinks) : "") + 
                "<a href=\"{0}\">{1}</a>".ToFormat("linktopostcomments", (commentCount + ((commentCount == 1) ? " comment" : " comments"))) +
